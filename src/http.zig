...
fn buildHeaders(
    ...
    try appendHeader(allocator, &headers, "User-Agent", "openrouter-zig/0.2.0");
...
test "prepare request adds required and optional headers" {
    ...
    try std.testing.expectEqualStrings("openrouter-zig/0.2.0", findHeader(prepared.headers, "user-agent").?);
